...
export async function insertProfile (profile: PrivateProfile): Promise<string> {
    ...
    //
    const { profileBio, profileActivationToken, profileEmail, profileHash, profileImage, profileFullName } = profile
    await sql`INSERT INTO profile(profile_id, profile_activation_token, profile_bio, profile_email, profile_hash, profile_image, profile_full_name) VALUES (gen_random_uuid(), ${profileBio}, ${profileActivationToken}, ${profileEmail}, ${profileHash}, ${profileImage}, ${profileFullName})`
    return 'Profile Successfully Created'
}
